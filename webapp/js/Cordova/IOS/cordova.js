...
},{"cordova":"cordova","cordova/channel":"cordova/channel","cordova/exec":"cordova/exec","cordova/utils":"cordova/utils"}],"cordova-plugin-statusbar.statusbar":[function(require,module,exports){
/*
 *
 * Licensed to the Apache Software Foundation (ASF) under one
 * or more contributor license agreements.  See the NOTICE file
...
module.exports.metadata = {
    ...
    "sino-cordova-plugin-cawrite": "1.0",
...
module.exports = [
    ...
    {
        "file": "www/ios/SinoCAWrite.js",
        "id": "sino-cordova-plugin-cawrite.SinoCAWrite",
        "name": "SinoCAWrite",
        "pluginId": "sino-cordova-plugin-cawrite",
        "clobbers": [
            "cordova.sino"
        ]
    },
...
},{"cordova/channel":"cordova/channel","cordova/platform":"cordova/platform"}],"sino-cordova-plugin-cawrite.SinoCAWrite":[function(require,module,exports){
var exec = require('cordova/exec');
...
module.exports = {
    setFormInfo:function(params,onSuccess,onError){
        var defaultOptions = {
            WONo : '',//work order 工单号 如果为空则自动生成
            FlowId : '1',//签名业务流程 先传个1吧 
            Channel : 'I',//渠道号 随便传吧
            IsSync : true,//是否同步归档系统
            serverEncType : 'RSA',//加密方式 RSA 和 SM2两种
        };

        params = extend(defaultOptions,params);
        exec(onSuccess,onError, 'SinoCAWrite', 'setFormInfo', [params]);
    },
    sign:function(params,onSuccess,onError){
        var defaultOptions = {
            contentId:20,//范围20-29 同一份文件 对于不同的签名 应使用不同的id
            name:'签名者姓名',
            idNumber:'证件号码',
            ruleType:'0',//签名图片与PDF合成时的方式 0:关键字匹配 1:坐标 2:规则
            tid:'10003',//规则号
            kw:'关键字',
            kwPos:'2',//关键字方向 具体什么含义还不知道
            kwOffset:'100',//关键字偏移量
            isTss:false,//时间戳
            geo:false,//获取地理位置
            left:'11.0',//坐标
        ...
        exec(onSuccess, onError, 'SinoCAWrite', 'showInputDialog', [params]);
    },
    signRiskTips:function(params,onSuccess,onError){
        var defaultOptions = {
            height:136,
            width:1360,
            commitment:'签字内容',
        ...
        params = extend(defaultOptions,params);
        exec(onSuccess,onError, 'SinoCAWrite', 'showMassInputDialog', [params]);
    },
    getData:function(onSuccess,onError){
        exec(onSuccess,onError, 'SinoCAWrite', 'getUploadDataGram', []);
    }
};
